 ...
 * @Author: Jackson_J
 * @CreateTime: 2019-01-12 12:13
 * @Description: ${Description}
 */
public class TableInnerQueryMain {
    public static void main(String[] args) throws Exception {
        // 1.创建一个 job = Mapper + Reduce
        Job job = Job.getInstance(new Configuration());

        // 指定Job 的入口
        job.setJarByClass(TableInnerQueryMain.class);

        // 指定任务的 Mapper 和输出的数据类型
        job.setMapperClass(TableInnerQueryMapper.class);
        job.setMapOutputKeyClass(LongWritable.class);    // key2 数据类型
        job.setMapOutputValueClass(Text.class); // value2 数据类型

        //指定任务的 Reducer
        job.setReducerClass(TableInnerQueryReducer.class);
